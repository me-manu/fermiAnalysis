...
#   when computing the log-likelihood distribution (the k-interval is such that sum Poisson(k,model_i) = 1-prob_epsilon).
...
def main(argv):
    ...
    help_msg = 'gtpsmap.py --cmap <data 3d map> --mmap <model 3d map> --wmap <weights 3d map (optional)> --outfile <outfile>\n           --emin <minimum energy/MeV; 1> --emax <maximum energy/MeV; 1e9>\n           --prob_epsilon <precision parameter; 1e-7>\n           --chatter <output verbosity; 1>'
    
    try:
        opts, args = getopt.getopt(argv,"h",["cmap=","mmap=","wmap=","outfile=","prob_epsilon=","fixedradius=","psfpar0=","psfpar1=","psfpar2=","psfpar3=","maxpoissoncount=","nbinpdf=","scaleaxis=","emin=","emax=","chatter=","ipix=","jpix=","rebin="])
    ...
    for opt, arg in opts:
        if opt == '-h':
            ...
            out_filename = arg
        elif opt in ("--fixedradius"):
            fixedradius = float(arg)
        elif opt in ("--psfpar0"):
            psfpar0 = float(arg)
        elif opt in ("--psfpar1"):
            psfpar1 = float(arg)
            ...
            psfpar3 = float(arg)
        elif opt in ("--maxpoissoncount"):
            maxpoissoncount = float(arg)
        elif opt in ("--prob_epsilon"):
            prob_epsilon = np.maximum(1e-15,float(arg))
        elif opt in ("--nbinpdf"):
            nbinpdf = int(arg)
        elif opt in ("--scaleaxis"):
            scaleaxis = float(arg)
        elif opt in ("--emin"):
            emin = float(arg)
        elif opt in ("--emax"):
    ...
    if dat_filename == '':
        print ('cmap is needed')
        print ('%s' % (help_msg))
        sys.exit()
    if mod_filename == '':
        print ('mmap is needed')
        print ('%s' % (help_msg))
        sys.exit()
    if out_filename == '':
        print ('outfile is needed')
        print ('%s' % (help_msg))
        sys.exit()
